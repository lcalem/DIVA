...
class VAEloc_encoder(nn.Module):
    def __init__(self, input_size, rnn_units, nlayers, bidirectional, batch_size, latent_dim, local_dim, pretrained, device):
        ...
        '''
        super(VAEloc_encoder, self).__init__()
...
class VAEloc_decoder(nn.Module):
    def __init__(self, input_size, rnn_units, fc_units, nlayers, latent_dim, local_dim, target_length, device):
        ...
        self.rnn_prediction = nn.GRU(input_size=input_size, hidden_size=rnn_units + local_dim + latent_dim, num_layers=nlayers, bidirectional=False, batch_first=True)
    ...
    def forward(self, z, past, hidden_init, l):
        '''
        ...
        decoder_hidden = torch.cat((l.unsqueeze(0), hidden_init, z.unsqueeze(0)), dim=2)
...
class cVAE_loc(nn.Module):
    ...
    def __init__(self,
        ...
        self.device = device
        self.encoder = VAEloc_encoder(input_size, rnn_units, nlayers, bidirectional, batch_size, latent_dim, local_dim, pretrained_enc, device)
    ...
    def forward(self, past, future, layout):
        ...
        x_mu = self.decoder(z, past, hidden_init, l)
    ...
    def sample(self, past, layout):
        ...
        with torch.no_grad():
            ...
            hidden_init, l = self.encoder.encode_past(past, layout)
