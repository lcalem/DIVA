...
def parse_options_file(filepath):
    ...
    if not filepath.startswith('/'):
        possible_paths = ['../config/%s' % filepath, '../config/old/%s' % filepath, '%s/physical-trajectory/config/%s' % (os.environ['HOME'], filepath), '%s/positional-priors/config/old/%s' % (os.environ['HOME'], filepath)]
